            ...
            switch($message->MsgType){
                case'event':
                    if($message->Event=='subscribe'){
                        $image = Image::make(base_path() .'/public/images/avatar/hlwelcome.png')
                        ->insert($userApi->get($message->FromUserName)->headimgurl, 'right');
                        return new News([
                        ...
                        switch($message->EventKey){
                            ...
                            case'HLJJ':
                                return new News([
                                    ...
                                    'description' => '浩立与华南理工大学共同携手合作，专业分子蒸馏、水蒸气蒸馏、超临界CO2萃取、超重力场。致力于：天然产物、香料、化工材料研发应用；分离提纯、化工生产、香料生产等设备设计制造。（给我们留言，我们客服将为您服务）',
                                    'url'         => 'http://www.hao-li.net/',
                                    'image'       => $userApi->get($message->FromUserName)->headimgurl,
                                ]);
                            case'RYZS':
